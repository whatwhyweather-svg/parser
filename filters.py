...
BUYER_INTENT_PATTERNS = [
    ...
    for p in (
        ...
        r"нужен\s+(?:сео|seo|сэо|сеошник|подрядчик|специалист|аудит|человек)",
        r"нужна\s+(?:помощь|семантика|сео|seo)",
        r"нужно\s+(?:сео|seo|продвин|аудит)",
...
CLEAR_HIRE_RE = re.compile(
    r"(?iu)(?:"
    r"ищу\s+(?:хорошего\s+)?(?:сео|seo|сэо)[\s\-]?шник|"
    r"ищу\s+(?:сео|seo|сэо)\s*специалист|"
    r"ищу\s+(?:сео|seo)\b|"
...
def looks_like_weak_seo_candidate(text: str, tag: str = "") -> bool:
    ...
    if not WEAK_LEAD_HINT_RE.search(raw):
        return False
    body = strip_hashtags(raw)
    # Тег поиска #SEO сам по себе НЕ делает лид — нужен SEO-маркер в тексте
    if not (
        has_seo_marker(body)
        ...
        return False
    # Нужен интент найма / боль, не просто слово «seo» в лекции
    if not (has_intent(raw) or has_search_pain(raw) or looks_like_clear_hire(raw)):
        return False
    return True
